...
두 **연속형 변수** 간의 연관성을 분석하는 모형. 범주형 변수 처리는 뒤에서 논할 예정이다.
...
선형회귀 모델을 만들기 전에 기준모델을 만들면 baseline을 알기 좋음
...
**특성을 고를 때 먼저 산점도를 그려봐서 선형관계 확인과 이상치 확인 작업 먼저 할 것**
...
x=X, y = e(y=y hat)로 그렸을 때, 

어떠한 패턴도 보이고 있지 않음-> 추정된 회귀식이 설명변수와 반은변수 간의 회귀모형을 적절하게 설명

x 값 증가할수록 잔차의 분산이 커지는 나팔관 형태-> 오차의 등분산 가정 위배(이분산), 회귀식이 설명변수와 반응변수의 관계를 적절하게 설명하지 못함

비선형관계, 이차 내지 고차방정식 유형->선형관계보다는 다항회귀나 비선형 모형으로 가정하고 재분석
