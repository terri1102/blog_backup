...
![barplot1](https://github.com/terri1102/blog_backup/blob/master/assets/images/barplot1.jpg?raw=true)
...
![barplot2](https://github.com/terri1102/blog_backup/blob/master/assets/images/barplot2.jpg?raw=true)
...
Plotly의 플롯은 두 종류가 있는데 axes-level인 플롯들 (boxplot, regplot, kdeplot 등)은 아래와 같이 명시적으로 ax를 표시해서 사용한다. 여기서 리턴되는 것은 axes 개체이다.
...
```
 ...
 반면 Figure-level 플롯들은 (lmplot, factorplot, jointplot 등) 다양한 axes를 가질 수 있으며 이를 명시하지 않아도 플롯을 그릴 수 있다. 이들이 리턴하는 것은 FacetGrid나 JointGrid 개체로 axes와 fig를 FacetGrid.axes를 통해서 접근할 수 있다.
